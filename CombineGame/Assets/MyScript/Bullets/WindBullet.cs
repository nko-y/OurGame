...
public class WindBullet : MonoBehaviourPun
{
    public int damage;
    public bool isMelee;
    public bool isRock;
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        if (!isMelee && other.gameObject.tag == "Player")
        {
            ...
            if (zp.Attribute == "Earth")
            {
                damage *= 2;
            }
            else if (zp.Attribute == "Fire")
            {
                damage /= 2;
            }
            ...
            p.RPC("OnSetWhiteRPC", RpcTarget.Others, 60, zp.name);
            p.RPC("OnPlayHitRPC", RpcTarget.All);
